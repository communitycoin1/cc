...
#
# Assumes it will talk to a dashd or Communitycoin-Qt running
# on localhost.
#
# Depends on jsonrpc
#
...
def determine_db_dir():
    """Return the default location of the Communitycoin Core data directory"""
...
def read_bitcoin_config(dbdir):
    """Read the communitycoin.conf file from dbdir, returns dictionary of settings"""
    ...
    config_parser.readfp(FakeSecHead(open(os.path.join(dbdir, "communitycoin.conf"))))
    return dict(config_parser.items("all"))

def connect_JSON(config):
    """Connect to a Communitycoin Core JSON-RPC server"""
    testnet = config.get('testnet', '0')
    testnet = (int(testnet) > 0)  # 0/1 in config file, convert to True/False
    if not 'rpcport' in config:
        config['rpcport'] = 8323 if testnet else 8283
...
def list_available(dashd):
    ...
    for output in unspent:
        ...
        # This code only deals with ordinary pay-to-communitycoin-address
...
def main():
    ...
    parser.add_option("--datadir", dest="datadir", default=determine_db_dir(),
                      help="location of communitycoin.conf file with RPC username/password (default: %default)")
