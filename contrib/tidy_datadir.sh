...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Communitycoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Communitycoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Communitycoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Communitycoin 0.7 datadir."
    ;;
  3)
    echo "Detected Communitycoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Communitycoin 0.8 datadir."
    ;;
esac
