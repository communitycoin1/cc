...
const char * const BITCOIN_CONF_FILENAME = "communitycoin.conf";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "communitycoin" is a composite category enabling all Communitycoin-related debug output
            if(ptrCategory->count(string("communitycoin"))) {
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty communitycoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override communitycoin.conf
